    ...
    public static class InstanceLauncherStartup
    {
        ...
        public static void AddInstanceLauncherStartupOptions(IConfiguration configuration)
        {
            ...
            if (string.IsNullOrEmpty(owsInstanceLauncherOptions.OWSAPIKey))
            {
                ...
                Console.Error.WriteLine("Please enter a valid OWSAPIKey in appsettings.json!");
            }
            ...
            {
                thereWasAStartupError = true;
                Console.Error.WriteLine("Please enter a valid PathToDedicatedServer in appsettings.json!");
            }
            ...
            {
                thereWasAStartupError = true;
                Console.Error.WriteLine("Your PathToDedicatedServer in appsettings.json points to a file that does not exist!  Please either point PathToDedicatedServer to your UE Editor exe or to your packaged UE dedicated server exe!");
            }
            ...
            else
            {
                    ...
                    OperatingSystem.IsLinux() && owsInstanceLauncherOptions.PathToDedicatedServer.EndsWith("Editor"))
                {
                    ...
                    if (testForUprojectPath.Count == 1)
                    {
                        ...
                        {
                            thereWasAStartupError = true;
                            Console.Error.WriteLine("Your PathToUProject in appsettings.json points to a uproject file that does not exist!");
                        }
                    }
                    else
                    {
                        thereWasAStartupError = true;
                        Console.Error.WriteLine("Because you are using UE4Editor.exe or UnrealEditor.exe, your PathToUProject in appsettings.json must contain a path to the uproject file.  Be sure to use escaped (double) backslashes in the path!");
                    }
                }
            }
            ...
            {
                Console.Error.WriteLine("Error encountered.  Shutting down...");
                Environment.Exit(-1);
            }
        }
